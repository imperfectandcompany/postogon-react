...
export class Start extends React.Component<{}, LoginState>{
  //set username and password to empty by default. We set loading as false since we aren't processing anything immediately.
    ...
    //we call this function everytime and input is changed to update the state. In this case, we are updating username and password
      ...
      handleSubmit(event:React.FormEvent<HTMLFormElement>) {
        ...
        axios.post('https://api.postogon.com/register',{
  ...
  <div className="mb-4">
    <label className="block mb-2 font-light text-md" htmlFor="email">Email</label>
    <input type="text" name="email" id="email" disabled={this.state.loading}  value={this.state.email} onChange={this.handleInputChange} autoComplete="off" className="w-full p-4 font-light leading-tight border border-gray-500 appearance-none bg-drabya-gray focus:outline-none focus:shadow-outline"/>
  </div>
  <div className="mb-4">
  ...
  <Link to="/continue"><div className="inline-block text-sm font-light text-indigo-600 align-baseline hover:text-indigo-500" >
