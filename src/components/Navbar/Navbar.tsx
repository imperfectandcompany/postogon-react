...
<div className="container flex items-center justify-between px-6 mx-auto">
                    ...
                    <div className="flex items-center">
						<svg aria-label="Home" className="w-8 sm:w-auto" id="logo" enableBackground="new 0 0 300 300" height="54" width="53" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 601 572">
                        ...
                        <p className="ml-2 text-base font-bold text-gray-800 lg:ml-4 lg:text-2xl">Postogon</p>
                    </div></Link>    
                    <div>
                        <div onClick={e => this.setState({toggleMenu: true})} className="text-gray-500 sm:block md:hidden lg:hidden hover:text-gray-700 focus:text-gray-700 focus:outline-none">
                            <svg aria-haspopup="true" aria-label="Main Menu" xmlns="http://www.w3.org/2000/svg" className="w-8 h-8" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z"></path>
                                <line x1="4" y1="8" x2="20" y2="8"></line>
                                <line x1="4" y1="16" x2="20" y2="16"></line>
                            </svg>
                        </div>
                        <div id="menu" className={this.state.toggleMenu ? 'md:block lg:block' : 'hidden md:block lg:block'}>
                            <div onClick={e => this.setState({toggleMenu: false})} className="fixed top-0 z-30 block pt-3 text-gray-500 md:hidden lg:hidden hover:text-gray-700 focus:text-gray-700 focus:outline-none">
      ...
      whileTap={{ scale: 0.95 }} className="pt-10 text-gray-600 cursor-pointer hover:text-gray-800 md:ml-10 md:pt-0">
                                <NavLink onClick={e => this.setState({toggleMenu: false})} exact={true} activeClassName='text-gray-800 hover:text-gray-900' to="/">
                                Home
                                </NavLink>
                                </motion.li>
                                <motion.li 
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.95 }} className="pt-10 text-gray-600 cursor-pointer hover:text-gray-800 md:ml-10 md:pt-0">
                                <NavLink onClick={e => this.setState({toggleMenu: false})} exact={true} activeClassName='text-gray-800 hover:text-gray-900' to="/start">
                                Start
                                </NavLink>
                                </motion.li>
                                <motion.li 
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.95 }} className="pt-10 text-gray-600 cursor-pointer hover:text-gray-800 md:ml-10 md:pt-0">
                                <NavLink onClick={e => this.setState({toggleMenu: false})} exact={true} activeClassName='text-gray-800 hover:text-gray-900' to="/continue">
                                Continue
                                </NavLink>
                                </motion.li>
                            </ul>
